...
- **折叠**：Collapse 模式框选一个已放置的预制体，折叠成单个 Composite 方块（内部模拟）；
  右键该方块展开还原。**Collapsed Place** 开启时，从组件栏拖出预制体直接以折叠单块放置。
  状态型组（含 DFF）暂不支持折叠。
...
3. 🚧 复合块：半加器 ✅、D 触发器 ✅、MUX ✅、DMUX 1→2 ✅；
   译码器 / 全加器被单层布线墙封锁（a 网 L 形 + 桩线横穿通道），
   已试：手写多布局、A*+拆线、候选路径回溯、宽预制体旋转——均失败；
   XOR 的 X 网（NAND(a,b)）被自身 a 网 L 形 + N3/b 完全围死无外部出口，
   暴露抽头（XOR_TAPPED ✅ 4/4）也无法引出——需重构 XOR 布局或在
   XOR 内规划 X 专用通道
